...
static func add_custom_project_setting(name: String, default_value, type: int, hint: int = PROPERTY_HINT_NONE, hint_string: String = "") -> void:
	...
	var setting_info: Dictionary = {"name": name, "type": type, "hint": hint, "hint_string": hint_string}
...
func create_setting() -> void:
	...
	add_custom_project_setting(panku_option(OPTIONS.DISABLE_ON_RELEASE), false, TYPE_BOOL)
