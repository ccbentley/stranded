...
@onready var normal_ui_group: Array = [group_toggle_button, state_control_button, rename_button]

@onready var edit_ui_group: Array = [rename_line_edit, confirm_rename_button, cancel_rename_button, move_up_button, move_down_button, remove_this_group_button]
...
func _ready():
	...
	state_control_button.button.toggled.connect(func(button_pressed: bool): state_control_button.icon = pause_icon if button_pressed else play_icon)
	...
	move_up_button.pressed.connect(func(): group_move_up.emit())

	move_down_button.pressed.connect(func(): group_move_down.emit())

	remove_this_group_button.pressed.connect(queue_free)

	load_persistent_data({"group_name": "Unnamed Group", "expressions": []})
...
func get_persistent_data() -> Dictionary:
	return {"group_name": group_toggle_button.text, "expressions": get_expressions(true)}
