...
func add_side_button(exp: String, icon: Texture2D, text: String, top := true):
	...
	side_button_separator.get_parent().move_child(new_button, side_button_separator.get_index() + int(!top))
	new_button.pressed.connect(func(): _repl._module.core.gd_exprenv.execute(exp))
