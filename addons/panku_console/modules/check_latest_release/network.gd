...
func check_latest_release():
	...
	if error != OK:
		is_busy = false
		response_received.emit({"success": false, "msg": "An error occurred in the HTTP request."})


func _on_request_completed(result: int, response_code: int, headers: PackedStringArray, body: PackedByteArray):
	is_busy = false
	if result != RESULT_SUCCESS:
		response_received.emit({"success": false, "msg": REQUEST_RESULT[result]})
		return
	var json = JSON.new()
	json.parse(body.get_string_from_utf8())
	var response: Dictionary = json.get_data()
	response_received.emit(
		{"success": true, "published_at": dget(response, "published_at", "???"), "name": dget(response, "name", "???"), "html_url": dget(response, "html_url", "???")}
	)
