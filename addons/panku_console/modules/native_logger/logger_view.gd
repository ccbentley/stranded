...
func add_log(message: String, level: int):
	...
	logs.push_back({"message": message, "level": level, "timestamp": Time.get_unix_time_from_system(), "count": 1})
...
func add_tag(filter_string: String):
	...
	tag.tag_btn.pressed.connect(func(): search(filter_string))
...
func _ready():
	#ui callbacks
	search_btn.pressed.connect(func(): search(search_box.text))
	search_box.text_submitted.connect(func(text: String): search(search_box.text))
