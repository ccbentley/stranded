...
func _ready():
	img_buffer = Image.create(CELL_SIZE * snake_game.MAP_SIZE, CELL_SIZE * snake_game.MAP_SIZE, false, Image.FORMAT_RGBA8)
	draw()
...
func draw():
	...
	for i in range(snake_game.snake_arr.size()):
		...
		img_buffer.fill_rect(Rect2i(s * CELL_SIZE, Vector2.ONE * CELL_SIZE), COLOR_SNAKE.sample(1.0 * i / snake_game.snake_arr.size()))

	# draw apple
	img_buffer.fill_rect(Rect2i(snake_game.apple * CELL_SIZE, Vector2.ONE * CELL_SIZE), COLOR_APPLE)
