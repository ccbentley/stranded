...
func add_snake_window():
	...
	window.size = Vector2(snake_ui.snake_game.MAP_SIZE * snake_ui.CELL_SIZE, 0)
	...
	snake_ui.snake_game.game_over.connect(
		func():
			var record = {"timestamp": Time.get_datetime_string_from_system(), "score": snake_ui.snake_game.get_snake_length()}
			leader_board_arr.append(record)
			leader_board_arr.sort_custom(func(a, b): return a["score"] > b["score"])
