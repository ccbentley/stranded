...
func _enter_tree() -> void:
	...
	get_editor_interface().get_script_editor().connect("editor_script_changed", _on_active_script_changed)
	_dockUI.plugin = self
	...
	while file_name != "":
		if dir.current_is_dir():
			...
			if (
				...
				or file_name.ends_with(".cs")
				or file_name.ends_with(".c")
				or file_name.ends_with(".cpp")
				or file_name.ends_with(".h")
