...
static func get_scene_export_defaults(node: Node) -> Dictionary:
	...
	if Engine.get_main_loop().has_meta("dialogic_scene_export_defaults") and node.script.resource_path in Engine.get_main_loop().get_meta("dialogic_scene_export_defaults"):
...
static func get_character_suggestions(
	_search_text: String, current_value: DialogicCharacter = null, allow_none := true, allow_all := false, editor_node: Node = null
) -> Dictionary:
	var suggestions := {}
