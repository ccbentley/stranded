...
func _refresh():
	...
	%JoinDefault.set_value(
		DialogicUtil.pretty_name(
			ProjectSettings.get_setting("dialogic/animations/join_default", get_script().resource_path.get_base_dir().path_join("DefaultAnimations/fade_in_up.gd"))
		)
	)
	%LeaveDefault.set_value(
		ProjectSettings.get_setting("dialogic/animations/leave_default", get_script().resource_path.get_base_dir().path_join("DefaultAnimations/fade_out_down.gd"))
	)
