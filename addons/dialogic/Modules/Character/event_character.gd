...
var regex := RegEx.create_from_string(
	'(?<type>join|update|leave)\\s*(")?(?<name>(?(2)[^"\\n]*|[^(: \\n]*))(?(2)"|)(\\W*\\((?<portrait>.*)\\))?(\\s*(?<position>\\d))?(\\s*\\[(?<shortcode>.*)\\])?'
)
	...
	match action:
		...
		Actions.UPDATE:
			...
			if animation_name:
				...
				var anim: DialogicAnimation = dialogic.Portraits.animate_character(character, animation_name, final_animation_length, final_animation_repitions)
...
#	add_header_button('', _on_character_edit_pressed, 'Edit character', ["ExternalLink", "EditorIcons"], 'character != null and character_identifier != "--All--"')
	...
	add_header_edit(
		"position", ValueType.NUMBER, {"mode": 1}, "character != null and !has_no_portraits() and action != %s and (action != Actions.UPDATE or set_position)" % Actions.LEAVE
	)
	...
	add_body_edit("animation_length", ValueType.NUMBER, {"left_text": "Length:", "suffix": "s"}, "should_show_animation_options() and !animation_name.is_empty()")
	...
	add_body_edit(
		"animation_repeats",
		ValueType.NUMBER,
		{"left_text": "Repeat:", "mode": 1},
		"should_show_animation_options() and !animation_name.is_empty() and action == %s)" % Actions.UPDATE
	)
	add_body_line_break()
	add_body_edit("position_move_time", ValueType.NUMBER, {"left_text": "Movement duration:"}, "action == %s and set_position" % Actions.UPDATE)
	add_body_edit(
		"set_z_index",
		ValueType.BOOL_BUTTON,
		...
		if CodeCompletionHelper.get_line_untill_caret(line).ends_with('animation="'):
			...
			for script in animations:
				TextNode.add_code_completion_option(
					CodeEdit.KIND_MEMBER, DialogicUtil.pretty_name(script), DialogicUtil.pretty_name(script) + '" ', TextNode.syntax_highlighter.normal_color
				)
