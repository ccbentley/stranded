...
func _execute() -> void:
	...
	if action == Actions.JOIN:
		...
		await dialogic.Portraits.join_character(character, portrait, transform, mirrored, z_index, extra_data, animation_name, final_animation_length, animation_wait)
	...
	# LEAVE -------------------------------------
		...
		if character_identifier == "--All--":
			...
			await dialogic.Portraits.leave_character(character, animation_name, final_animation_length, animation_wait)
...
func get_shortcode_parameters() -> Dictionary:
	return {
		"action":
		{
			...
			"default": 0,
			"custom_stored": true,
			"suggestions": func(): return {"Join": {"value": Actions.JOIN}, "Leave": {"value": Actions.LEAVE}, "Update": {"value": Actions.UPDATE}}
		},
		"character":
		{
		...
		"move_ease": {"property": "transform_ease", "default": Tween.EaseType.EASE_IN_OUT, "suggestions": func(): return list_to_suggestions(ease_options)},
		"move_trans": {"property": "transform_trans", "default": Tween.TransitionType.TRANS_SINE, "suggestions": func(): return list_to_suggestions(trans_options)},
...
func build_event_editor() -> void:
	add_header_edit(
		...
		ValueType.FIXED_OPTIONS,
		{
			"options":
			[
				{"label": "Join", "value": Actions.JOIN, "icon": load("res://addons/dialogic/Editor/Images/Dropdown/join.svg")},
				{"label": "Leave", "value": Actions.LEAVE, "icon": load("res://addons/dialogic/Editor/Images/Dropdown/leave.svg")},
				{"label": "Update", "value": Actions.UPDATE, "icon": load("res://addons/dialogic/Editor/Images/Dropdown/update.svg")}
			]
		}
	)
	add_header_edit(
	...
	add_header_edit(
		...
		{"icon": load("res://addons/dialogic/Modules/Character/update_portrait.svg"), "tooltip": "Change Portrait"},
	...
	add_header_edit(
		"set_transform",
		ValueType.BOOL_BUTTON,
		{"icon": load("res://addons/dialogic/Modules/Character/update_position.svg"), "tooltip": "Change Position"},
	...
	add_body_edit(
		"fade_animation",
		ValueType.DYNAMIC_OPTIONS,
		{"left_text": "Fade:", "suggestions_func": get_fade_suggestions, "editor_icon": ["Animation", "EditorIcons"], "placeholder": "Default", "enable_pretty_name": true},
		"should_show_fade_options()"
	)
	add_body_edit("fade_length", ValueType.NUMBER, {"left_text": "Length:", "suffix": "s", "min": 0}, "should_show_fade_options() and !fade_animation.is_empty()")
	...
	add_body_edit("animation_length", ValueType.NUMBER, {"left_text": "Length:", "suffix": "s", "min": 0}, "should_show_animation_options() and !animation_name.is_empty()")
	add_body_edit("animation_wait", ValueType.BOOL, {"left_text": "Await end:"}, "should_show_animation_options() and !animation_name.is_empty()")
	add_body_edit(
		"animation_repeats",
		ValueType.NUMBER,
	...
	add_body_line_break()
	add_body_edit("transform_time", ValueType.NUMBER, {"left_text": "Movement duration:", "min": 0}, "should_show_transform_options()")
	...
	add_body_edit(
		"set_z_index",
		ValueType.BOOL_BUTTON,
		{"icon": load("res://addons/dialogic/Modules/Character/update_z_index.svg"), "tooltip": "Change Z-Index"},
		"character != null and action == Actions.UPDATE"
	)
	add_body_edit("z_index", ValueType.NUMBER, {"left_text": "Z-index:", "mode": 1}, "action != %s and (action != Actions.UPDATE or set_z_index)" % Actions.LEAVE)
	add_body_edit(
		"set_mirrored",
		ValueType.BOOL_BUTTON,
		{"icon": load("res://addons/dialogic/Modules/Character/update_mirror.svg"), "tooltip": "Change Mirroring"},
		"character != null and action == Actions.UPDATE"
	)
	add_body_edit("mirrored", ValueType.BOOL, {"left_text": "Mirrored:"}, "action != %s and (action != Actions.UPDATE or set_mirrored)" % Actions.LEAVE)
				...
				"animation":
					...
					for script: String in animations:
						TextNode.add_code_completion_option(
							CodeEdit.KIND_VARIABLE, DialogicUtil.pretty_name(script), DialogicUtil.pretty_name(script), TextNode.syntax_highlighter.normal_color, null, '" '
						)
