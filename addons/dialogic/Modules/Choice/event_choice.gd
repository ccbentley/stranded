...
func from_text(string: String) -> void:
	...
	if result.get_string("shortcode"):
		...
		else_action = {"default": ElseActions.DEFAULT, "hide": ElseActions.HIDE, "disable": ElseActions.DISABLE}.get(shortcode_params.get("else", ""), ElseActions.DEFAULT)
...
func build_event_editor() -> void:
	...
	add_body_edit("disabled_text", ValueType.SINGLELINE_TEXT, {"left_text": "Disabled text:", "placeholder": "(Empty for same)"}, "allow_alt_text()")


func allow_alt_text() -> bool:
	return (
		condition
		and (else_action == ElseActions.DISABLE or (else_action == ElseActions.DEFAULT and ProjectSettings.get_setting("dialogic/choices/def_false_behaviour", 0) == 1))
	)
