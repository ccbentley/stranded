...
func get_base_content_size() -> Vector2:
	...
	return font.get_multiline_string_size(text.get_parsed_text(), HORIZONTAL_ALIGNMENT_LEFT, max_width, text.get_theme_font_size(&"normal_font_size"))
...
func get_speaker_canvas_position() -> Vector2:
	if node_to_point_at:
		if node_to_point_at is Node3D:
			base_position = get_viewport().get_camera_3d().unproject_position((node_to_point_at as Node3D).global_position)
