...
func _on_continued_revealing_text(new_character: String) -> void:
	...
	audio_player.pitch_scale = max(
		0, current_overwrite_data.get("pitch_base", base_pitch) + current_overwrite_data.get("pitch_variance", pitch_variance) * RNG.randf_range(-1.0, 1.0)
	)
	audio_player.volume_db = (
		current_overwrite_data.get("volume_base", base_volume) + current_overwrite_data.get("volume_variance", volume_variance) * RNG.randf_range(-1.0, 1.0)
	)
