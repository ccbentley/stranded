...
func show_next_indicators(question := false, autoadvance := false) -> void:
	for next_indicator in get_tree().get_nodes_in_group("dialogic_next_indicator"):
		if next_indicator.enabled:
			if (
				(question and "show_on_questions" in next_indicator and next_indicator.show_on_questions)
				or (autoadvance and "show_on_autoadvance" in next_indicator and next_indicator.show_on_autoadvance)
...
func update_text_speed(
	letter_speed: float = -1, absolute := false, speed_multiplier := _speed_multiplier, user_speed: float = dialogic.Settings.get_setting("text_speed", 1)
) -> void:
	if letter_speed == -1:
...
func effect_mood(text_node: Control, skipped: bool, argument: String) -> void:
	if argument.is_empty():
		return
	if dialogic.current_state_info.get("speaker", ""):
		update_typing_sound_mood(load(dialogic.current_state_info.speaker).custom_info.get("sound_moods", {}).get(argument, {}))
