...
var timeline_identifier := "":
	...
	set(value):
		...
		if not timeline_identifier in DialogicResourceUtil.get_label_cache().keys() or not label_name in DialogicResourceUtil.get_label_cache()[timeline_identifier]:
			label_name = ""
...
func get_shortcode_parameters() -> Dictionary:
	return {
		...
		"timeline": {"property": "timeline_identifier", "default": null, "suggestions": get_timeline_suggestions},
...
func build_event_editor() -> void:
	...
	add_header_edit(
		"label_name",
		ValueType.DYNAMIC_OPTIONS,
		{"left_text": "at", "empty_text": "the beginning", "suggestions_func": get_label_suggestions, "editor_icon": ["ArrowRight", "EditorIcons"]}
	)
...
func _get_code_completion(CodeCompletionHelper: Node, TextNode: TextEdit, line: String, _word: String, symbol: String) -> void:
	...
	if symbol == "/":
		CodeCompletionHelper.suggest_labels(
			TextNode,
			line.strip_edges().trim_prefix("jump ").trim_suffix("/" + String.chr(0xFFFF)).strip_edges(),
			"\n",
			event_color.lerp(TextNode.syntax_highlighter.normal_color, 0.6)
		)
