...
func load_layer(layer_id := ""):
	...
	load_layout_scene_customization(layer_info.path, layer_info.overrides, inherited_layer_info.overrides)
...
func _on_add_layer_menu_pressed(index: int) -> void:
	...
	# Adding a custom scene as a layer
		...
		find_parent("EditorView").godot_file_dialog(_on_add_custom_layer_file_selected, "*.tscn, Scenes", EditorFileDialog.FILE_MODE_OPEN_FILE, "Open custom layer scene")
...
func _on_replace_layer_menu_pressed(index: int) -> void:
	...
	# Adding a custom scene as a layer
		...
		find_parent("EditorView").godot_file_dialog(_on_replace_custom_layer_file_selected, "*.tscn, Scenes", EditorFileDialog.FILE_MODE_OPEN_FILE, "Open custom layer scene")
...
func _on_make_custom_menu_pressed(index: int) -> void:
	# This layer only
	if index == 2:
		find_parent("EditorView").godot_file_dialog(_on_make_custom_layer_file_selected, "", EditorFileDialog.FILE_MODE_OPEN_DIR, "Select folder for new copy of layer")
	# The full layout
	if index == 3:
		find_parent("EditorView").godot_file_dialog(_on_make_custom_layout_file_selected, "", EditorFileDialog.FILE_MODE_OPEN_DIR, "Select folder for new layout scene")
