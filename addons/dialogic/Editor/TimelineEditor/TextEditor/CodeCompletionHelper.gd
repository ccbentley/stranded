			...
			for shortcode in shortcodes:
				...
				if shortcode_events[shortcode].get_shortcode_parameters().is_empty():
					text.add_code_completion_option(
						...
						shortcode,
						shortcode,
						shortcode_events[shortcode].event_color.lerp(syntax_highlighter.normal_color, 0.3),
				...
				for param in parameters:
					if !param + "=" in full_event_text:
						text.add_code_completion_option(
							...
							param,
							param + '="',
							shortcode_events[code].event_color.lerp(syntax_highlighter.normal_color, 0.3),
...
func suggest_portraits(text: CodeEdit, character_name: String, end_check := ")") -> void:
	...
	if character_resource.portraits.is_empty():
		text.add_code_completion_option(
			CodeEdit.KIND_MEMBER, "Has no portraits!", "", syntax_highlighter.character_portrait_color, load("res://addons/dialogic/Editor/Images/Pieces/warning.svg")
		)
