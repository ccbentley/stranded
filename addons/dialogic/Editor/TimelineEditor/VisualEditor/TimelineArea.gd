...
func _process(delta: float) -> void:
	...
	for child in %Timeline.get_children():
		if (child.global_position.y < get_global_mouse_position().y) and (child.global_position.y + child.size.y > get_global_mouse_position().y):
...
func _draw() -> void:
	...
	for idx in range($Timeline.get_child_count()):
		...
		if block.resource.wants_to_group:
			...
			draw_rect(
				Rect2(rect_position.x - global_position.x - line_width, rect_position.y - global_position.y - icon_panel_height / 2, line_width, line_width), group_color
			)
			...
			if group_starter:
				...
				## Draw vertical line
				draw_rect(Rect2(rect_position.x - global_position.x + offset.x, rect_position.y - global_position.y + offset.y, line_width, v_length), group_color)
