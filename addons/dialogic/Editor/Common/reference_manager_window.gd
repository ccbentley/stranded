...
func add_ref_change(
	old_name: String, new_name: String, type: Types, where := Where.TEXTS_ONLY, character_names := [], whole_words := false, case_sensitive := false, previous: Dictionary = {}
) -> void:
	var regexes := []
	var category_name := ""
	match type:
		...
		Types.VARIABLE:
			regexes = [r"{(?<replace>\s*%s\s*)}" % old_name.replace("/", "\\/"), r'var\s*=\s*"(?<replace>\s*%s\s*)"' % old_name.replace("/", "\\/")]
			category_name = "Variables"

		Types.PORTRAIT:
			regexes = [r"(?m)^[^:(\n]*\((?<replace>%s)\)" % old_name.replace("/", "\\/"), r"\[\s*portrait\s*=(?<replace>\s*%s\s*)\]" % old_name.replace("/", "\\/")]
			category_name = "Portraits by " + character_names[0]

		Types.CHARACTER_NAME:
			# for reference: ((join|leave|update) )?(?<replace>NAME)(?!\B)(?(1)|(?!([^:\n]|\\:)*(\n|$)))
			regexes = [r"((join|leave|update) )?(?<replace>%s)(?!\B)(?(1)|(?!([^:\n]|\\:)*(\n|$)))" % old_name]
	...
	broken_manager.reference_changes.insert(
		idx,
		{
			"what": old_name,
			"forwhat": new_name,
