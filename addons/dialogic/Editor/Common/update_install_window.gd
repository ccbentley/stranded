...
func markdown_to_bbcode(text: String) -> String:
	...
	while res:
		text = text.replace(
			res.get_string(), "[code][color=" + get_theme_color("accent_color", "Editor").to_html() + "]" + res.get_string("text").strip_edges() + "[/color][/code]"
		)
		res = small_code_regex.search(text)
	...
	while res:
		text = text.replace(
			res.get_string(),
			"[code][bgcolor=" + get_theme_color("box_selection_fill_color", "Editor").to_html() + "]" + res.get_string("text").strip_edges() + "[/bgcolor][/code]"
		)
		res = big_code_regex.search(text)
