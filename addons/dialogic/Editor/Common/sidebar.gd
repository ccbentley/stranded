...
func update_resource_list(resources_list: PackedStringArray = []) -> void:
	...
	match group_mode:
		...
		GroupMode.PATH:
			...
			sorted_dir_keys.sort_custom(func(x, y): return x.get_slice("/", x.get_slice_count("/") - 1) < y.get_slice("/", y.get_slice_count("/") - 1))
...
class ResourceListItem:
	...
	func _to_string() -> String:
		return JSON.stringify(
			{"text": text, "index": index, "icon": icon.resource_path, "metadata": metadata, "tooltip": tooltip, "parent_dir": get_parent_directory()}, "\t", false
		)
...
func _on_right_click_menu_id_pressed(id: int) -> void:
	match id:
		...
		2:  # OPEN IN FILESYSTEM
			EditorInterface.get_file_system_dock().navigate_to_path(%RightClickMenu.get_meta("item_clicked").get_metadata(0))
		3:  # OPEN IN EXTERNAL EDITOR
			OS.shell_open(ProjectSettings.globalize_path(%RightClickMenu.get_meta("item_clicked").get_metadata(0)))
		4:  # COPY IDENTIFIER
			DisplayServer.clipboard_set(DialogicResourceUtil.get_unique_identifier(%RightClickMenu.get_meta("item_clicked").get_metadata(0)))
